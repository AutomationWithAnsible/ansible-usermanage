...
extra_advanced_extra_db = cat_remote_file("/tmp/extra_advanced_extra_db.yaml")

describe "Database match for extra advanced" do

  it "extra variables" do
    expected_hash = [{"name"=> "label", "openvpn"=> true, "state"=> "present" },{ "name"=> "ops", "openvpn"=> true, "state"=> "present"},{"name"=> "tool", "state"=> "present", "zendesk"=> true, "zendesk_options"=> {"group"=> "Admin" }}]
    given_hash = YAML.load(extra_advanced_extra_db)
